...
def get_typedef(winapi, winapi_csv):
    ...
    for function_name in winapi:
        ...
        function_ptr.append(
            f'{function_name.upper()} {function_name}_ptr = ({function_name.upper()})ptrGetProcAddress({dll_name}_mod, "{function_name}");\n'
        )

    for dll in dlls:
        hmodule.append(f'HMODULE {dll}_mod = ptrLoadLibraryA("{dll}.dll");\n')
...
def insert_code(input_file, typedef, hmodule, function_ptr, output_file):
    ...
    peb_code = [
        "HMODULE kernel32baseAddr = getkernel32baseAddr();\n",
        'GETPROCADDRESS ptrGetProcAddress = (GETPROCADDRESS)GetProcAddressKernel32(kernel32baseAddr, "GetProcAddress");\n',
        'LOADLIBRARYA ptrLoadLibraryA = (LOADLIBRARYA)GetProcAddressKernel32(kernel32baseAddr, "LoadLibraryA");\n',
    ]
    lines = (
        lines[:main_block]
        + ["\n"]
...
def main():
    ...
    parser.add_argument("--input", required=True, help="Input .cpp file")
    parser.add_argument("--output", default="output.cpp", help="Output .cpp file")
    parser.add_argument(
